...
\usepackage{amsmath,amssymb,amsth_doc}
...
% Custom command to replace \boxed
\newcommand{\important}[1]{
  \par\noindent\color{MyDarkBlue}\textit{#1}\par
}
...
\noindent
\textit{% 
  Here, we include all the process on this topic. We will start by putting the Primary example, and step by step go on.
}
\end{abstract}
...
\)
and
\(e_{ij}^{\,\ell}M_{n_i}e_{ij}^{\,\ell}\)
inherits the matrix–algebra structure of $M_{m_j}$ through
\((\theta_{ij}^{\,\ell})^{-1}\). 
Explicitly, we may pick *matrix units* \(
  \{u^{(\ell)}_{ab}\}_{a,b=1}^{m_j}
\subset M_{n_i}
\)
so that
...
Define
\important{
   $e_B : L^2(A)\longrightarrow L^2(A)$,
   \quad
   $e_B(\xi)=E_\tau(\xi)$
   \qquad(\(\xi\in A\subseteq L^2(A)\)).
}
...
inclusions
\important{ $B \ \subseteq\ A \ \subseteq\ A_1$ }
...
\important{
    $[A:B]=\|H\|_2^{\,2}=\sum_{i=1}^{s}\sum_{j=1}^{r}h_{ij}^{\,2}.$
}
...
\[
  A \;\cong\; \bigl(p_1Ap_1\bigr)\oplus\cdots\oplus\bigl(p_sAp_s\bigr), \quad
...
\[
  ...
  \underbrace{\begin{bmatrix}
     \beta &         &         &         &         \\
           ...
           &         &         & B_{2}   &         \\
           &         &         &         & B_{2}
  \end{bmatrix}}_{M_{9}(\mathbb{C})\text{ — four }2\times2\text{ blocks}}.
\]
...
\[
  E(\alpha\oplus A_{9})=
  \Bigl[\tfrac13(\Tr\alpha)+\tfrac19 a_{11}\Bigr]
  \;\oplus\; 
  \frac{1}{4}
  \sum_{k=1}^4 \begin{pmatrix} a_{2k, 2k} & a_{2k, 2k+1} \\ a_{2k+1, 2k} & a_{2k+1, 2k+1} \end{pmatrix}
\]
where the sum is over the four $2 \times 2$ blocks in $M_9(\mathbb{C})$.
...
\[
  ...
  :=\Bigl\{\,x\in\mathcal B(H)\;\Big|\;
      \exists\,a\in\mathcal B\bigl(H[-1,n]\bigr):\;
      x(\alpha,\beta)
      =\delta_{\alpha_{[-1,n]},\,\beta_{[-1,n]}}\, 
      a\bigl(\alpha_{[n+1,\infty)},\beta_{[n+1,\infty)}\bigr)
  \Bigr\}.
\]
Then
...
\[
  ...
  \delta_{\alpha_{[n+1,\infty)},\,\beta_{[n+1,\infty)}}\, 
  \sqrt{\frac{t^{(n)}_{\,r(\alpha_n)}}
       {t^{(n-1)}_{\,r(\alpha_{n-1})}}}\, 
  \delta_{\alpha_{[n,n]},\,\beta_{[n,n]}}.
\]
\end{definition}
...
\[
e_{2}(\alpha, \beta) = \delta_{\alpha_{[-1,0]}, \beta_{[-1,0]}} \, \delta_{\alpha_{[2, \infty)}, \beta_{[2, \infty)}} \, \sqrt{ \frac{t^{(1)}_{r(\alpha_1)}}{t^{(0)}_{r(\alpha_0)}} } \, \delta_{\alpha_{[1,1]}, \beta_{[1,1]}}
\]
where for \( \alpha, \beta \in \Omega[-1,2] \):
- \( \alpha = (\alpha_0, \alpha_1, \alpha_2) \) where \( \alpha_0 \) is a path from level -1 to 0, etc.
- \( e_2(\alpha, \beta) \neq 0 \) only if the paths agree up to level 0 and after level 1.
\medskip

\subsection*{Step 3: Weights}
The Markov trace weights for the vertices (simple blocks) are proportional to the dimensions of the corresponding simple modules. Let's normalize $t^{(-1)}=1$.
\begin{itemize}
    \item Level 0: \( \dimvec{m} = (1, 2)^T \). Let trace vector \( t^{(0)} = (1, 2)^T \).
    \item Level 1: \( \dimvec{n} = (3, 9)^T \). Let trace vector \( t^{(1)} = (3, 9)^T \).
    \item Level 2: \( \dimvec{n_2} = (18, 36)^T \). Let trace vector \( t^{(2)} = (18, 36)^T \).
\end{itemize}
The coefficients for the square root term are:
\begin{itemize}
    \item Path \( p_1 \to q_1 \): \( \sqrt{ \frac{t^{(1)}_{q_1}}{t^{(0)}_{p_1}} } = \sqrt{ \frac{3}{1} } = \sqrt{3} \).
    \item Path \( p_1 \to q_2 \): \( \sqrt{ \frac{t^{(1)}_{q_2}}{t^{(0)}_{p_1}} } = \sqrt{ \frac{9}{1} } = 3 \).
    \item Path \( p_2 \to q_1 \): (not possible as $h_{12}=0$).
    \item Path \( p_2 \to q_2 \): \( \sqrt{ \frac{t^{(1)}_{q_2}}{t^{(0)}_{p_2}} } = \sqrt{ \frac{9}{2} } = \frac{3}{\sqrt{2}} \).
\end{itemize}
\medskip
...
\[
e_{2} = \begin{bmatrix} I_3 & 0 \\
0 & 0 \end{bmatrix}_{18 \times 18} \oplus \begin{bmatrix} 1 & 0 \\
0 & 0 \end{bmatrix}_{36 \times 36}.
\]
